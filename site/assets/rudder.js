...
(function () {
  ...
  /* типы рулей: балансирная доля и расчётная схема балки «руль—баллер».
     A1/A = 0 у небалансирного навесного руля (перо целиком за осью),
     0,20 у полуподвесного и 0,25 у подвесного балансирного — характерные
     значения по Правилам; k5/k7 — коэффициенты схемы балки для числа опор. */
  const RTYPE = {
    '1': { name: 'простой навесной (на рудерпосте)', a1: 0.00, k5: 0.125, k7: 1.0, sup: 'петли на рудерпосте' },
    '3': { name: 'полуподвесной на кронштейне', a1: 0.20, k5: 0.158099, k7: 0.715489, sup: 'штырь в пятке / кронштейн' },
    's': { name: 'подвесной балансирный', a1: 0.25, k5: 0.250, k7: 0.5, sup: 'только баллер (консоль)' },
  };
  function rtype() { const e = $('in-rtype'); return RTYPE[(e && e.value) || '3'] || RTYPE['3']; }
  ...
  function compute(i) {
    ...
    r.M2 = r.F * C.hb * i.rt.k5 / (8 * i.rt.k7) / 1000;
  ...
  function renderSteps(i, r) {
    ...
    s.push(step(3, 'Балансировка',
      ...
      `${i.rt.name}: A₁/A = ${N(i.rt.a1, 2)} → плечо момента (0,35 − A₁/A) = ${N(0.35 - i.rt.a1, 2)}`));
    ...
    s.push(step(9, 'Крутящий момент на переднем ходу (п. 2.2.2.3)',
      ...
      `= ${N(r.F,1)}·(${N(r.A,0)}/${N(i.h,1)})·(0,35 − ${N(i.rt.a1,2)})`,
  ...
  if ($('in-rtype')) $('in-rtype').addEventListener('change', update);
  update();
})();
