...
(function () {
  ...
  function compute(i) {
    ...
    if (!(r.l2 > 0)) return r;
    const l1 = i.l1cm / 100;
    // ----- сетка d1 (стандартный ряд диаметров) -----
  ...
  function render(i, r) {
    ...
    s.push(step(4, 'Второе приближение — подбор d₁ сеткой',
      ...
      `<div style="overflow-x:auto;max-height:340px;overflow-y:auto">
       ...
       ${gRows}</table></div>
       <span class="muted">«—» — потери превышают напор либо нарушены ограничения; формы записи потерь — по учебной методике курса</span>`,
